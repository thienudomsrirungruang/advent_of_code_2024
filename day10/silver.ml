...
let rec reachable (grid: int array array) (pos: Pos.t): PosSet.t =
  let current = grid.(pos.x).(pos.y) in
  if current = 9 then
    PosSet.singleton pos
  else
    let proc_arm dir =
      ...
      if inbounds grid next && grid.(next.x).(next.y) = current + 1 then
        ...
        PosSet.empty
    in
    List.fold_left (fun acc dir -> PosSet.union acc (proc_arm dir)) PosSet.empty dirs
  ...
  for i = 0 to n - 1 do
    for j = 0 to m - 1 do
      if inputs.(i).(j) = 0 then
        ans := !ans + (PosSet.cardinal (reachable inputs (Pos.make i j)))
    done
  done;
  print_int !ans;
  print_newline ()
