  ...
  for i = 0 to n - 1 do
    for j = 0 to m - 1 do
      ...
      if not (get is_wall p) then
        dirs |> List.iter (fun dir ->
          let q = Vec2.add p (Vec2.mul dir 2) in
          if inbounds is_wall q && not (get is_wall q) then
            let shortcut_dist = get start_dist p + 2 + get goal_dist q in
            if shortcut_dist <= no_shortcut_dist - good_threshold then
              incr ans;
              (* Printf.printf "Shortcut found at (%d, %d) that saves %d\n" i j (no_shortcut_dist - shortcut_dist) *)
        )
    done
  done;
  Printf.printf "%d\n" !ans
